...
const Pricing = () => {
  ...
  const plans = [
    {
      ...
      popular: false,
      founderPrice: null as string | null,
    },
    {
      key: "standard",
  ...
  const comparisonRows = [
    {
      label: t("pricing.tablePhotos"),
      values: [
        t("pricing.tableFreePhotos"),
        t("pricing.tableStandardPhotos"),
        t("pricing.tableProPhotos"),
        t("pricing.tableElitePhotos"),
      ],
    },
    {
      label: t("pricing.tableSearch"),
      values: [
        t("pricing.tableFreeSearch"),
        t("pricing.tableStandardSearch"),
        t("pricing.tableProSearch"),
        t("pricing.tableEliteSearch"),
      ],
    },
    {
      label: t("pricing.tableAvailableNow"),
      values: [
        t("pricing.tableFreeAvailableNow"),
        t("pricing.tableStandardAvailableNow"),
        t("pricing.tableProAvailableNow"),
        t("pricing.tableEliteAvailableNow"),
      ],
    },
    {
      label: t("pricing.tableTravel"),
      values: [
        t("pricing.tableFreeTravel"),
        t("pricing.tableStandardTravel"),
        t("pricing.tableProTravel"),
        t("pricing.tableEliteTravel"),
      ],
    },
    {
      label: t("pricing.tableAnalytics"),
      values: [
        t("pricing.tableFreeAnalytics"),
        t("pricing.tableStandardAnalytics"),
        t("pricing.tableProAnalytics"),
        t("pricing.tableEliteAnalytics"),
      ],
    },
    {
      label: t("pricing.tableExtras"),
      values: [
        t("pricing.tableFreeExtras"),
        t("pricing.tableStandardExtras"),
        t("pricing.tableProExtras"),
        t("pricing.tableEliteExtras"),
      ],
    },
  ];
  ...
  return (
    <div className="min-h-screen bg-background" ref={scrollRef}>
      <SEOHead
        ...
        description="Compare Free, Standard, Pro, and Elite advertising plans for male massage therapists on MasseurMatch."
        path="/pricing"
      />
      <CursorGlow />
      <ScrollProgress />
      ...
      <section className="pt-28 pb-16">
          ...
          <div className="container mx-auto px-4 mb-6">
            <div className="max-w-3xl mx-auto flex items-center gap-3 rounded-lg border border-accent/30 bg-accent/10 px-4 py-3">
              ...
              <p className="text-sm text-muted-foreground">
                Subscription info is temporarily unavailable. Prices shown are standard rates and your actual plan will appear once the system reconnects.
              </p>
            </div>
          </div>
        )}
        ...
        <div className="container mx-auto px-4">
          ...
          >
            ...
            <p className="text-sm text-muted-foreground">Try any paid plan risk-free. You will only be asked for payment when your trial ends.</p>
          </motion.div>
          ...
          >
            ...
            <ul className="space-y-3">
              {addOns.map((addon) => (
                <li key={addon} className="flex items-start gap-2 text-sm">
          ...
          >
            <div className="flex items-center gap-2 mb-5">
              ...
              <h2 className="text-2xl font-bold text-foreground">{t("pricing.comparisonTitle")}</h2>
            </div>
            <div className="overflow-x-auto rounded-lg border border-border bg-background">
              <table className="min-w-full text-sm">
                <thead className="bg-muted/40">
                  <tr>
                    <th className="px-4 py-3 text-left font-semibold text-foreground">{t("pricing.tableTier")}</th>
                    {plans.map((plan) => (
                      <th key={plan.key} className="px-4 py-3 text-left font-semibold text-foreground">
                        <div className="flex items-center gap-2">
                          <span>{plan.name}</span>
                          {plan.popular && <Badge className="text-[10px]">Most Popular</Badge>}
                        </div>
                        <p className="mt-1 text-xs font-normal text-muted-foreground">{plan.price}/{plan.period}</p>
                      </th>
                    ))}
                  </tr>
                </thead>
                  ...
                  {comparisonRows.map((row) => (
                    <tr key={row.label} className="border-t border-border">
                      ...
                      {row.values.map((value, index) => (
                        <td key={`${row.label}-${plans[index].key}`} className="px-4 py-3 text-muted-foreground">
                          {value}
                        </td>
                      ))}
                    </tr>
          ...
          <div className="max-w-3xl mx-auto mt-32">
            ...
            <div className="space-y-px bg-border">
              {[
                ...
                <motion.div key={faq.q} custom={index} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} className="bg-background p-8">
