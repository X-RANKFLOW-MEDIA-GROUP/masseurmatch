...
const TherapistProfile = () => {
  ...
  const specialties = useMemo(() => (profile?.specialties || []) as string[], [profile?.specialties]);
  const certifications = useMemo(() => (profile?.certifications || []) as string[], [profile?.certifications]);
  const langs = useMemo(() => (profile?.languages || []) as string[], [profile?.languages]);
  const socialMedia = (profile?.social_media || {}) as Record<string, string>;
  const businessHours = useMemo(
    () => (profile?.business_hours || {}) as Record<string, Record<string, string>> | Record<string, string>,
  ...
  const incallHours = useMemo(
    () => (hasStructuredHours ? (businessHours as Record<string, Record<string, string>>).incall || {} : businessHours as Record<string, string>),
    [businessHours, hasStructuredHours]
  );
  ...
  const customFaq = useMemo(() => (profile?.custom_faq || []) as { question: string; answer: string }[], [profile?.custom_faq]);
  const pricingSessions = useMemo(() => (profile?.pricing_sessions || []) as { name: string; duration: number; incall: number; outcall: number }[], [profile?.pricing_sessions]);
  ...
  const jsonLdItems = useMemo(() => {
    ...
    return items;
  }, [businessHours, cityLabel, customFaq, displayName, hasStructuredHours, id, incallHours, langs, pricingSessions, primaryPhoto, profile, specialties]);
  ...
  }, [certifications, cityLabel, displayName, langs, pricingSessions, profile, specialties]);
        ...
        <article className="max-w-5xl mx-auto" itemScope itemType="https://schema.org/HealthAndBeautyBusiness">
          ...
          {mapCenter && profile.city && (
            <motion.section initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="border border-border bg-card p-8 md:p-10 mb-8 rounded-lg" aria-labelledby="location-heading">
              ...
              <p className="text-sm text-muted-foreground mb-6">Based in {cityLabel}. Exact location is shared directly by the therapist after contact.</p>
