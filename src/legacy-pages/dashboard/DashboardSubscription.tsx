...
const DashboardSubscription = () => {
  ...
  const handleCheckout = async (planKey: string) => {
    ...
    try {
      ...
      toast({ title: "Checkout error", description: getErrorMessage(error, "Failed to create checkout session"), variant: "destructive" });
    } finally {
  ...
  const handleManage = async () => {
    ...
    try {
      ...
      toast({ title: "Portal error", description: getErrorMessage(error, "Failed to open portal"), variant: "destructive" });
    } finally {
  ...
  const currentPlanKey = normalizePlanKey(subscription.plan_key) || (isSubscribed ? "standard" : null);
        ...
        <div className="glass-card p-4 border border-success/20 bg-success/5">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">Limited to the first 50 members. All paid plans include a 14-day free trial with no card required during trial.</p>
            </div>
          </div>
        </div>
      )}
            ...
            <div key={plan.key} className={`glass-card p-6 space-y-4 ${plan.popular ? "ring-1 ring-primary" : ""} ${isCurrent ? "border-l-4 border-l-success" : ""}`}>
              ...
              {isCurrent ? (
                ...
                >
                  ...
                  {isSubscribed ? "Switch Plan" : plan.isFree ? "Start Free" : "Start Free Trial"}
