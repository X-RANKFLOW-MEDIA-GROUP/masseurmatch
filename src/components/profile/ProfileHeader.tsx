import { BadgeCheck, Crown, MapPin, Sparkles } from "lucide-react";
  ...
  return (
    <div className="rounded-[24px] border border-white/5 bg-[#101C2B]/90 p-6 shadow-2xl backdrop-blur-xl transition-all duration-300 hover:-translate-y-0.5">
      <div className="flex flex-wrap items-center gap-2 text-sm font-medium text-[#94A3B8]">
        {profile.isVerified && <span className="inline-flex items-center gap-1 rounded-full border border-emerald-400/20 bg-emerald-400/10 px-3 py-1 text-emerald-300"><BadgeCheck className="h-4 w-4" />Verified</span>}
        {profile.isFeatured && <span className="inline-flex items-center gap-1 rounded-full border border-amber-400/20 bg-amber-400/10 px-3 py-1 text-amber-300"><Sparkles className="h-4 w-4" />Featured</span>}
      ...
      <div className="mt-6 flex flex-wrap gap-3 text-sm text-[#94A3B8]">
        <span className="inline-flex items-center gap-2"><MapPin className="h-4 w-4 text-[#3B82F6]" />{profile.neighborhood}, {profile.city}, {profile.state}, {profile.country}</span>
