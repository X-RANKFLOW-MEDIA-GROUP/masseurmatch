...
export function SeoContentSections({ profile }: { profile: ProfileViewModel }) {
  ...
  return <div className="space-y-8">
    ...
    <Card id="trust" title="Trust and Profile Details"><dl className="grid gap-3 md:grid-cols-2"><div><dt className="text-[#64748B]">Verified</dt><dd className="text-white">{profile.isVerified ? "Yes" : "Not listed"}</dd></div><div><dt className="text-[#64748B]">Premium</dt><dd className="text-white">{profile.isPremium ? "Yes" : "Not listed"}</dd></div><div><dt className="text-[#64748B]">Featured</dt><dd className="text-white">{profile.isFeatured ? "Yes" : "Not listed"}</dd></div><div><dt className="text-[#64748B]">Member since</dt><dd className="text-white">{profile.memberSince}</dd></div><div><dt className="text-[#64748B]">Response time</dt><dd className="text-white">{profile.responseTime}</dd></div><div><dt className="text-[#64748B]">Last active</dt><dd className="text-white">{profile.lastActiveAt}</dd></div><div><dt className="text-[#64748B]">Profile status</dt><dd className="text-white">{profile.profileStatus}</dd></div></dl></Card>
  </div>;
}
