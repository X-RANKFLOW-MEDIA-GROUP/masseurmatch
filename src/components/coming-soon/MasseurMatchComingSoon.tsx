...
function PointerGlow() {
  ...
  return <motion.div aria-hidden="true" style={{ x: springX, y: springY, position: "fixed", zIndex: 0, width: 280, height: 280, borderRadius: "50%", background: "radial-gradient(circle, rgba(255,138,31,.16), transparent 66%)", pointerEvents: "none", mixBlendMode: "screen" }} />;
}
...
function MagneticButton({ children, disabled }: { children: React.ReactNode; disabled?: boolean }) {
  ...
  return (
    <motion.button type="submit" disabled={disabled} onMouseMove={handleMove} onMouseLeave={() => { x.set(0); y.set(0); }} style={{ x: springX, y: springY, minHeight: 58, border: 0, borderRadius: 999, padding: "0 26px", color: C.dark, background: `linear-gradient(135deg, ${C.orange}, ${C.amber})`, fontFamily: C.sans, fontWeight: 800, letterSpacing: "0.01em", cursor: disabled ? "not-allowed" : "pointer", boxShadow: "0 18px 54px rgba(255,138,31,.28)", opacity: disabled ? 0.72 : 1, display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 10 }} whileTap={{ scale: 0.98 }}>
      {children}
    </motion.button>
  );
}
    ...
    <MotionConfig reducedMotion="user">
      <main style={{ background: C.dark, color: C.white, minHeight: "100vh", overflow: "hidden", position: "relative", fontFamily: C.sans }}>
        ...
        <div style={{ position: "absolute", inset: 0, pointerEvents: "none" }}>
          ...
          <motion.div animate={{ y: [-18, 18, -18], scale: [1, 1.04, 1] }} transition={{ duration: 9, repeat: Infinity, ease: "easeInOut" }} style={{ position: "absolute", right: "-180px", top: "-180px", width: 620, height: 620, borderRadius: "50%", background: "rgba(255,138,31,0.15)", filter: "blur(120px)" }} />
          <motion.div animate={{ y: [16, -16, 16], scale: [1, 1.06, 1] }} transition={{ duration: 11, repeat: Infinity, ease: "easeInOut", delay: 1.3 }} style={{ position: "absolute", left: "-150px", bottom: "-120px", width: 540, height: 540, borderRadius: "50%", background: "rgba(255,138,31,0.58)", filter: "blur(110px)" }} />
        </div>
