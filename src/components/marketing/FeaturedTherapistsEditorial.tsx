  ...
  return (
    <section className="relative overflow-hidden bg-gradient-to-b from-[#f7f7f7] to-[#ffffff] py-24 lg:py-32">
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        >
          <div>
            <p className="font-mono text-xs uppercase tracking-[0.3em] text-[#8B1E2D]">Featured profiles</p>
            <h2 className="mt-4 font-display text-4xl font-black tracking-tight text-[#111111] sm:text-5xl">
              Featured Therapists to Explore
            </h2>
            <p className="mt-4 max-w-2xl text-sm leading-6 text-[#6F6F6F]">
              Featured is a promotional placement. Identity verification, when completed, is shown separately on each profile.
            </p>
          </div>
          <Link
            href="/search"
          ...
          >
            Browse all <ArrowRight size={14} className="inline" />
          </Link>
        </motion.div>
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {featuredTherapists.map((therapist, index) => {
            ...
            const isVerified = therapist.is_verified_identity === true;
                ...
                >
                  <div className="relative aspect-[3/4] overflow-hidden bg-[#F7F7F7]">
                    {therapist.profile_photo || therapist.avatar_url ? (
                      ...
                      <div className="flex h-full items-center justify-center font-display text-6xl font-extrabold text-gray-200">{initials}</div>
                    )}
                    {isVerified ? (
                      <div className="absolute left-4 top-4 flex items-center gap-1.5 rounded-full bg-black/70 px-3 py-1.5 text-[10px] font-bold uppercase tracking-wider text-white backdrop-blur-sm">
                        <BadgeCheck size={12} className="text-white" /> Identity Verified
                      </div>
                    ) : null}
                    <div className="absolute right-4 top-4 flex items-center gap-1 rounded-full bg-gradient-to-r from-[#8B1E2D] to-[#A82E3D] px-3 py-1.5 text-[10px] font-bold uppercase tracking-wider text-white shadow-lg">
                      <Star size={11} className="fill-white" strokeWidth={2.5} /> Featured
                    </div>
                  </div>

                  <div className="flex flex-col p-6">
                    <h3 className="font-display text-lg font-black leading-tight text-[#111111]">{therapist.display_name || therapist.full_name || "Therapist"}</h3>
                    <p className="mt-1.5 text-sm font-medium text-[#8B1E2D]">{buildLocationLabel(therapist)}</p>
                    {specialties.length > 0 ? (
                      <div className="mt-4 flex flex-wrap gap-2">
                        {specialties.map((s) => (
                          <span key={s} className="rounded-full border border-[#8B1E2D]/20 bg-gradient-to-r from-[#F8EDEE] to-[#FCF4F6] px-3 py-1.5 text-xs font-semibold text-[#8B1E2D]">{s}</span>
                        ))}
                      </div>
                    ) : null}
                    <div className="mt-auto flex items-center justify-between border-t border-[#E8E8E8] pt-4">
                      <p className="text-xs font-bold uppercase tracking-wide text-[#8B1E2D]">{formatPrice(therapist.incall_price || therapist.outcall_price)}</p>
