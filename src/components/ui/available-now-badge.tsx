...
export const AvailableNowBadge = ({ size = "md", className }: AvailableNowBadgeProps) => {
  ...
  return (
    ...
    >
      {/* Outer glow ring */}
      <span className="absolute inset-0 rounded-full animate-ping bg-success/10 pointer-events-none" />
      {/* Soft shadow glow */}
      <span
        className="absolute -inset-[2px] rounded-full pointer-events-none"
        style={{
          boxShadow: "0 0 12px 2px hsl(var(--success) / 0.35), 0 0 24px 4px hsl(var(--success) / 0.15)",
        }}
      />
