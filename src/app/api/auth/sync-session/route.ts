...
/**
 * Legacy endpoint retained for compatibility. With Supabase SSR the browser
 * client writes the auth cookies directly on sign-in, so there is nothing to
 * "sync". This simply confirms the current cookie session and makes sure the
 * profile/role rows exist, then returns the resolved role.
 */
