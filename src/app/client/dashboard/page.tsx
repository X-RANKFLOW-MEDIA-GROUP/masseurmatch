              ...
              {upcomingAppts.map((appt: any) => (
                <li key={appt.id} className="py-3 flex items-center gap-4">
                  {appt.therapist?.avatar_url && (
                    <img src={appt.therapist.avatar_url} alt="" className="w-10 h-10 rounded-full object-cover" />
                  )}
                  <div className="flex-1">
                    <p className="font-medium text-gray-800">{appt.therapist?.full_name}</p>
              ...
              {pastAppts.map((appt: any) => (
                <li key={appt.id} className="py-3 flex items-center gap-4">
                  ...
                  {appt.status === 'completed' && (
                    <Link
                      href={`/therapists/${appt.therapist?.slug}?review=1&appointment=${appt.id}`}
