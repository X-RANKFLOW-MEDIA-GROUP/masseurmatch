    ...
    <section>
      ...
      <div className="rounded-2xl border border-[#E8E8E8] bg-white p-4 shadow-sm">
        <div className="mb-3 flex items-center gap-2">
          <SlidersHorizontal className="h-4 w-4 text-[var(--color-primary)]" strokeWidth={2.25} />
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <div className="relative">
            ...
            <input
              ...
              className="w-full rounded-xl border border-[#E8E8E8] bg-[#FAFAFA] py-2.5 pl-9 pr-3 text-sm text-[#111111] placeholder:text-[#8E8E8E] focus:border-[var(--color-primary)]/50 focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)]/20"
            />
          </div>
          <div className="relative">
            <Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[#8E8E8E]" strokeWidth={2} />
      ...
      <div className="mt-5 flex items-center justify-between gap-4">
        <p className="text-sm text-[#5a5147]" aria-live="polite">
          ...
          {city ? <> near <span className="font-medium text-[var(--color-primary)]">{city}</span></> : ""}
        </p>
        {hasFilters && (
          <button
            type="button"
            onClick={clearAll}
            className="flex items-center gap-1.5 text-sm font-semibold text-[var(--color-primary)] hover:text-[var(--color-primary-hover)]"
          >
      ...
      {items.length > 0 ? (
        ...
        <div className="mt-6 rounded-3xl border border-dashed border-[#E8E8E8] px-6 py-12 text-center">
          <MapPin className="mx-auto mb-3 h-8 w-8 text-[var(--color-primary)]/40" strokeWidth={1.5} />
          ...
          <button
            type="button"
            onClick={clearAll}
            className="mt-4 inline-flex items-center gap-2 rounded-full bg-[var(--color-primary)] px-5 py-2 text-sm font-semibold text-white"
          >
            <X className="h-3.5 w-3.5" strokeWidth={2.5} />
            Clear filters
          </button>
      ...
      {totalPages > 1 && (
        <div className="mt-8 flex items-center justify-center gap-3">
          <button
            ...
            className="rounded-xl border border-[#E8E8E8] bg-white px-4 py-2 text-sm font-semibold text-[#111111] shadow-sm disabled:opacity-40 hover:border-[var(--color-primary)]/40"
          >
            Prev
          </button>
