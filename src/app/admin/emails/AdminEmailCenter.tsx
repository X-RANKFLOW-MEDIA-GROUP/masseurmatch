...
export default function AdminEmailCenter() {
  ...
  const generateWithAi = async () => {
    ...
    try {
      ...
      setNotice({ ok: true, text: "AI draft generated. Review the audience, content and schedule before sending." });
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-2xl border border-violet-200 bg-gradient-to-br from-violet-50 via-white to-slate-50 p-6 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h2 className="font-semibold text-slate-900">AI Email Generator</h2>
              <p className="mt-1 max-w-3xl text-sm text-slate-600">Describe the objective and MasseurMatch AI creates the campaign name, subject, email HTML, plain text and delivery suggestions. Nothing is sent automatically.</p>
            </div>
          </div>
          <span className="inline-flex items-center gap-1.5 rounded-full bg-white px-3 py-1 text-xs font-semibold text-violet-700 ring-1 ring-violet-200"><Bot className="h-3.5 w-3.5" /> Draft only</span>
        </div>

        <div className="mt-5 grid gap-4 xl:grid-cols-[minmax(0,1.5fr)_minmax(260px,.5fr)]">
          <div>
            <Field label="What should this email accomplish?">
              <textarea value={aiObjective} onChange={(e) => setAiObjective(e.target.value)} rows={4} className="input" placeholder="Example: Encourage providers with incomplete profiles to finish their profile and explain why complete information helps clients understand their services." />
            </Field>
            <div className="grid gap-4 md:grid-cols-2">
              <Field label="Audience"><input value={aiAudience} onChange={(e) => setAiAudience(e.target.value)} className="input" placeholder="Free providers with incomplete profiles" /></Field>
              ...
              <Field label="CTA"><input value={aiCta} onChange={(e) => setAiCta(e.target.value)} className="input" placeholder="Complete your profile" /></Field>
              <Field label="Offer or announcement"><input value={aiOffer} onChange={(e) => setAiOffer(e.target.value)} className="input" placeholder="Optional. Only factual details." /></Field>
            </div>
          </div>
          <div className="flex flex-col justify-between rounded-xl border border-violet-100 bg-white p-4">
            <div className="space-y-3 text-sm text-slate-600">
              <p className="font-semibold text-slate-900">AI safety layer</p>
              <p>Uses the existing server side LLM provider chain.</p>
              <p>Blocks unsupported claims about bookings, licensing, earnings and services.</p>
              <p>Generated content always returns to the composer for human review.</p>
            </div>
            <button type="button" onClick={() => void generateWithAi()} disabled={generating || aiObjective.trim().length < 3} className="mt-5 inline-flex items-center justify-center gap-2 rounded-xl bg-violet-700 px-5 py-3 text-sm font-semibold text-white disabled:opacity-50">
              {generating ? <Loader2 className="h-4 w-4 animate-spin" /> : <Sparkles className="h-4 w-4" />} {generating ? "Generating..." : "Generate email"}
            </button>
          </div>
        </div>
        ...
        {aiMeta ? (
          <div className="mt-4 grid gap-3 md:grid-cols-3">
            <AiNote label="Preview text" value={aiMeta.previewText || "Not suggested"} />
            <AiNote label="Suggested audience" value={aiMeta.suggestedAudience || aiAudience} />
            <AiNote label="Suggested timing" value={aiMeta.suggestedSchedule || "Review and schedule manually"} />
          </div>
        ) : null}
      </section>
        ...
        <section className="overflow-hidden rounded-2xl border border-border bg-white shadow-sm">
          <div className="border-b border-border p-5">
            <div className="flex items-center justify-between gap-3">
              ...
              <button type="button" onClick={selectVisible} className="text-sm font-semibold text-brand-secondary hover:underline">Select visible</button>
            </div>
            <div className="relative mt-4">
              <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" />
              <input value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => event.key === "Enter" && void load(query)} placeholder="Search people..." className="w-full rounded-xl border border-slate-200 py-2.5 pl-9 pr-12 text-sm outline-none focus:border-brand-secondary" />
              <button type="button" onClick={() => void load(query)} className="absolute right-2 top-1/2 -translate-y-1/2 rounded-lg p-1.5 text-slate-500 hover:bg-slate-100"><RefreshCw className="h-4 w-4" /></button>
            </div>
            ...
            {loading ? (
              ...
              <label key={recipient.userId} className="flex cursor-pointer gap-3 rounded-xl p-3 hover:bg-slate-50">
                ...
                <span className="min-w-0 flex-1">
                  <span className="flex items-center gap-2"><span className="truncate text-sm font-semibold text-slate-900">{recipient.name}</span>{recipient.suppressed || !recipient.marketingOptIn ? <AlertTriangle className="h-3.5 w-3.5 shrink-0 text-amber-500" /> : null}</span>
        ...
        <div className="space-y-6">
          <section className="rounded-2xl border border-border bg-white p-6 shadow-sm">
            <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                ...
                <div><h2 className="font-semibold text-slate-900">Campaign composer</h2><p className="text-sm text-muted-foreground">Create newsletters, announcements and targeted campaigns through the compliant lifecycle queue.</p></div>
              </div>
            ...
            <div className="mt-5 flex flex-wrap items-center justify-between gap-3">
              <p className="max-w-2xl text-xs text-muted-foreground">Variables: <code>{"{{name}}"}</code> and <code>{"{{city}}"}</code>. Marketing sends enforce opt out, cooldown, suppression and unsubscribe rules.</p>
              <div className="flex flex-wrap gap-2">
                <button type="button" onClick={() => void saveTemplate()} disabled={saving || !subject || !bodyHtml} className="inline-flex items-center gap-2 rounded-xl border border-slate-200 px-4 py-2.5 text-sm font-semibold text-slate-700 disabled:opacity-50">{saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />} Save template</button>
                <button type="button" onClick={() => void createCampaign()} disabled={!canSend} className="inline-flex items-center gap-2 rounded-xl bg-brand-secondary px-5 py-2.5 text-sm font-semibold text-white disabled:opacity-50">{sending ? <Loader2 className="h-4 w-4 animate-spin" /> : scheduledFor ? <CalendarClock className="h-4 w-4" /> : <Send className="h-4 w-4" />}{scheduledFor ? "Schedule campaign" : "Queue campaign"}</button>
              </div>
            </div>
          </section>
          ...
          <section className="rounded-2xl border border-border bg-white p-6 shadow-sm">
            ...
            <div className="overflow-hidden rounded-xl border border-slate-200">
              <div className="border-b border-slate-200 bg-slate-50 px-5 py-3 text-sm"><strong>Subject:</strong> {(subject || "No subject").replaceAll("{{name}}", "Provider").replaceAll("{{city}}", "your city")}</div>
              {aiMeta?.previewText ? <div className="border-b border-slate-200 px-5 py-2 text-xs text-slate-500"><strong>Preview text:</strong> {aiMeta.previewText}</div> : null}
              <iframe title="Email HTML preview" sandbox="" srcDoc={previewHtml} className="min-h-[360px] w-full bg-white" />
            </div>
          </section>
        </div>
      </div>
      ...
      <section className="rounded-2xl border border-border bg-white p-6 shadow-sm">
        <div className="mb-4 flex items-center gap-3"><FileText className="h-5 w-5 text-brand-secondary" /><div><h2 className="font-semibold text-slate-900">Campaign history</h2><p className="text-sm text-muted-foreground">Live delivery totals from the lifecycle email system.</p></div></div>
      ...
      <section className="grid gap-4 md:grid-cols-3">
        <Capability icon={Sparkles} title="AI generation" text="Generate complete newsletter and campaign drafts directly inside Email Center." />
        <Capability icon={ShieldCheck} title="Delivery safeguards" text="Suppression, preferences, cooldown and unsubscribe controls remain in the delivery layer." />
        <Capability icon={CalendarClock} title="Lifecycle ready" text="The existing lifecycle workers continue to handle automated provider communication independently of manual campaigns." />
      </section>

      <style jsx>{`
        :global(.input) { width: 100%; border-radius: 0.75rem; border: 1px solid rgb(226 232 240); padding: 0.625rem 0.75rem; font-weight: 400; outline: none; background: white; }
...
function AiNote({ label, value }: { label: string; value: string }) {
  return <div className="rounded-xl border border-violet-100 bg-white p-3"><p className="text-xs font-semibold uppercase tracking-wide text-violet-700">{label}</p><p className="mt-1 text-sm text-slate-700">{value}</p></div>;
}

function Capability({ icon: Icon, title, text }: { icon: typeof Mail; title: string; text: string }) {
  return <div className="rounded-2xl border border-border bg-white p-5 shadow-sm"><div className="flex items-start gap-3"><div className="rounded-lg bg-slate-100 p-2 text-slate-600"><Icon className="h-4 w-4" /></div><div><p className="font-semibold text-slate-900">{title}</p><p className="mt-1 text-sm text-slate-600">{text}</p></div></div></div>;
}
