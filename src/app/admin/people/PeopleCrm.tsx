...
export default function PeopleCrm({ people }: { people: AdminPerson[] }) {
  ...
  const updateProfile = (person: AdminPerson, action: "approve" | "reject" | "verify_identity" | "feature" | "unfeature") => run(
            ...
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-slate-100">
                {filtered.map((person) => {
                  ...
                  return (
                    <tr key={person.profileId} onClick={() => setSelectedId(person.profileId)} className={`cursor-pointer transition ${active ? "bg-slate-950 text-white" : "hover:bg-slate-50"}`}>
              ...
              <div className="space-y-5">
                ...
                <div className="rounded-2xl border border-slate-200 bg-white p-4">
                  ...
                  <div className="mt-3 flex gap-2">
                    <Button className="flex-1 bg-emerald-600 hover:bg-emerald-700" onClick={() => void updateProfile(selected, "approve")}><Check className="mr-2 h-4 w-4" />Approve</Button>
                    <Button className="flex-1" variant="destructive" onClick={() => void updateProfile(selected, "reject")}><X className="mr-2 h-4 w-4" />Reject</Button>
                  </div>
                </div>
