  ...
  return (
    <div className="min-h-screen bg-[#1A1A1A] p-6">
      <div className="mx-auto max-w-4xl">
        <div className="mb-8">
          <p className="font-mono text-[10px] uppercase tracking-[0.18em] text-[#C8102E]">Your Dashboard</p>
              ...
              <div className="flex flex-col gap-2 max-h-40 overflow-y-auto">
                {inq.ai_conversation.map((msg, i) => (
                  <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-[80%] rounded-xl px-3 py-1.5 text-xs ${msg.role === 'user' ? 'bg-[#C8102E]/20 text-red-200' : 'bg-white/[0.06] text-slate-300'}`}>
                      {msg.content}
                    </div>
                  </div>
                ))}
