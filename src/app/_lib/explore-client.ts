// Client-safe explore utilities (no server-only imports)
...
export function parseExploreSearchParams(params: SearchParamShape): ExploreFilters {
  ...
  return {
    city: getFirstParam(params.city),
    zip,
...
// Client-safe city coordinate and resolution utilities
...
export function resolveExploreCityWithData(
  inputCity: string,
  cities: CityData[],
  inputZip = "",
) {
...
export function getCityCoordinatesFromSlug(citySlug: string) {
  return CITY_COORDINATES[citySlug] || US_CENTER;
}
