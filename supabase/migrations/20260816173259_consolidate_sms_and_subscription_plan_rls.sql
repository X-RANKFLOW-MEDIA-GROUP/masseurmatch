...
create policy sms_logs_admin_insert on public.sms_logs
  ...
  with check (exists (select 1 from public.user_roles ur where ur.user_id = (select auth.uid()) and ur.role = 'admin'::text));
create policy sms_logs_admin_update on public.sms_logs
  for update to authenticated
  using (exists (select 1 from public.user_roles ur where ur.user_id = (select auth.uid()) and ur.role = 'admin'::text))
  with check (exists (select 1 from public.user_roles ur where ur.user_id = (select auth.uid()) and ur.role = 'admin'::text));
create policy sms_logs_admin_delete on public.sms_logs
  for delete to authenticated
  using (exists (select 1 from public.user_roles ur where ur.user_id = (select auth.uid()) and ur.role = 'admin'::text));

-- sms_profiles: old effective predicate for every action was provider-owner OR admin.
...
create policy sms_profiles_owner_or_admin_all on public.sms_profiles
  for all to authenticated
  using (
    profile_id in (select p.id from public.profiles p where p.user_id = (select auth.uid()))
    or exists (select 1 from public.user_roles ur where ur.user_id = (select auth.uid()) and ur.role = 'admin'::text)
  )
  with check (
    profile_id in (select p.id from public.profiles p where p.user_id = (select auth.uid()))
    or exists (select 1 from public.user_roles ur where ur.user_id = (select auth.uid()) and ur.role = 'admin'::text)
...
-- subscription_plans: preserve anonymous active-plan reads and authenticated active OR admin reads;
-- writes remain admin-only with the exact existing admin predicate.
drop policy if exists "Admins can manage subscription plans" on public.subscription_plans;
drop policy if exists "Public can read active subscription plans" on public.subscription_plans;
create policy subscription_plans_anon_read_active on public.subscription_plans
  for select to anon
  using (is_active = true);
create policy subscription_plans_authenticated_read_active_or_admin on public.subscription_plans
  for select to authenticated
  using (
    is_active = true
    or exists (
...
create policy subscription_plans_admin_insert on public.subscription_plans
  for insert to authenticated
  with check (exists (select 1 from public.profiles p where p.id = (select auth.uid()) and p.role = 'admin'::text));
