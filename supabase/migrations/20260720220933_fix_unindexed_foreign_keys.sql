...
-- ========================================
--
-- Several of these indexes target production-only tables/columns that no
-- committed migration creates (analytics_events, appointments.profile_id,
-- complaints, moderation_actions, sms_*, therapist_pricing/services/
-- subscriptions, waitlist_voice_ai, …). On a fresh database (Supabase branch
-- preview, local db reset) those are absent, so a single `CREATE INDEX` aborted
-- the whole migration and blocked every migration after it.
--
-- Run each statement independently and skip any whose table/column is absent
-- (a missing FK index is a performance-only concern, never a correctness one).
-- On production, where every object exists, all indexes are still created.
...
BEGIN
  FOREACH stmt IN ARRAY stmts LOOP
    ...
    EXCEPTION WHEN undefined_table OR undefined_column THEN
      RAISE NOTICE 'fix_unindexed_foreign_keys: skipped (missing table/column): %', stmt;
    END;
  END LOOP;
END $$;
