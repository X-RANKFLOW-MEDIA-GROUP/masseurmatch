...
drop policy if exists moderation_queue_insert_self on public.moderation_queue;
...
with duplicate_pending as (
  select id,
         row_number() over (
           partition by item_type, source, target_id
...
delete from public.moderation_queue mq
