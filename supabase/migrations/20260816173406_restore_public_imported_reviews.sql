...
set is_public = true,
    public_label = coalesce(nullif(btrim(public_label), ''), 'Imported review'),
    updated_at = now()
...
select
  ...
  ir.review_text,
  ir.review_date,
  coalesce(nullif(btrim(ir.public_label), ''), 'Imported review') as public_label,
  ir.imported_at,
  ir.created_at
from public.imported_reviews ir
where ir.is_public = true
  and ir.reviewed_at is not null
  and ir.review_text is not null
  and btrim(ir.review_text) <> '';
...
comment on view public.public_imported_reviews is
  'Safe server-side projection of reviewed imported reputation for public provider profiles. Source URLs and private migration metadata are intentionally excluded.';
