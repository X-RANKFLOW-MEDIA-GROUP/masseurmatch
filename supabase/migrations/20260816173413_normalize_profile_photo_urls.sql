...
as $$
begin
  if (new.storage_path is null or new.storage_path !~* '^https?://')
     and new.url ~* '^https?://' then
    new.storage_path := new.url;
  end if;

  if (new.url is null or new.url !~* '^https?://')
     and new.storage_path ~* '^https?://' then
...
create trigger trg_normalize_profile_photo_urls
before insert or update of storage_path, url on public.profile_photos
for each row execute function public.normalize_profile_photo_urls();
