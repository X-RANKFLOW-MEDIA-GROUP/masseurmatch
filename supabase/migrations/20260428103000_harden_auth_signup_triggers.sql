...
-- Keep helper function safe even when public.users does not exist yet.
...
-- Remove every known legacy trigger name that can block auth signup.
...
BEGIN
  ...
  -- Best effort public.users mirror.
  BEGIN
...
EXCEPTION
  WHEN OTHERS THEN
    -- Absolute safety net: never block Supabase Auth from creating the user.
...
-- Keep compatibility aliases for older references without attaching more triggers.
