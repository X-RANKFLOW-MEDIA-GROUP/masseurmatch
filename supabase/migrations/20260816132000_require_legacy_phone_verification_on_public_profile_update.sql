-- Keep legacy public profiles visible, but require phone verification before
-- any future update. This closes direct-API and alternate-client bypasses while
-- preserving the existing listing until the provider next accesses or edits it.
...
begin
  ...
  if tg_op = 'INSERT' then
    ...
    must_verify := new.visibility_status = 'public'
      and (
        ...
        or coalesce(old.is_verified_phone, false) = false
      );
  end if;
  ...
  if coalesce(new.is_verified_phone, false) = false then
    raise exception using
      errcode = '23514',
      message = 'Phone verification must be completed before a public provider profile can be updated.';
  end if;
